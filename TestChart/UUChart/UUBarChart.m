...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    float level = (_yValueMax-_yValueMin) /Y_COUNT;
    CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;
    CGFloat levelHeight = chartCavanHeight /Y_COUNT;
...
-(void)strokeChart
{
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            titleLable.frame = CGRectMake(0, (1 - grade) * chartCavanHeight - 10, _xLabelWidth * (_yValues.count==1?0.8:0.45), _xLabelWidth * (_yValues.count==1?0.8:0.45));
            titleLable.textColor = [UIColor blackColor];
            titleLable.text = valueString;
